...
import net.corda.fxsettlement.states.RecordedTradeStateTests;
...
public class TradeIssueTests {
    ...
    static private final MockServices ledgerServices = new MockServices(
            Arrays.asList("net.corda.fxsettlement.states")
    );
    ...
    public void issueTransactionMustHaveNoInputs() {
        ...
        ledger(ledgerServices, l -> {
            l.transaction(tx -> {
                tx.input(RecordedTradeContract.IOU_CONTRACT_ID, iou);
                tx.command(Arrays.asList(TestUtils.ALICE.getPublicKey(), TestUtils.BOB.getPublicKey()),
                        new RecordedTradeContract.Commands.Issue());
                tx.output(RecordedTradeContract.IOU_CONTRACT_ID, iou);
                return tx.failsWith("No inputs should be consumed when issuing a trade record");
            });
            l.transaction(tx -> {
                tx.output(RecordedTradeContract.IOU_CONTRACT_ID, iou);
                tx.command(Arrays.asList(TestUtils.ALICE.getPublicKey(), TestUtils.BOB.getPublicKey()),
    ...
    public void issueTransactionMustHaveOneOutput() {
        ...
        ledger(ledgerServices, l -> {
            l.transaction(tx -> {
                tx.command(Arrays.asList(TestUtils.ALICE.getPublicKey(), TestUtils.BOB.getPublicKey()),
                        new RecordedTradeContract.Commands.Issue());
                tx.output(RecordedTradeContract.IOU_CONTRACT_ID, iou); // Two outputs fails.
                tx.output(RecordedTradeContract.IOU_CONTRACT_ID, iou);
                return tx.failsWith("Only one output states should be created when issuing a trade record.");
            });
            l.transaction(tx -> {
                tx.command(Arrays.asList(TestUtils.ALICE.getPublicKey(), TestUtils.BOB.getPublicKey()),
                        new RecordedTradeContract.Commands.Issue());
                tx.output(RecordedTradeContract.IOU_CONTRACT_ID, iou); // One output passes.
                return tx.verifies();
            });
            return null;
        });
    ...
    public void cannotCreateZeroValueIOUs() {
        ledger(ledgerServices, l -> {
            l.transaction(tx -> {
                ...
                tx.output(RecordedTradeContract.IOU_CONTRACT_ID,
                ...
                return tx.failsWith("A newly issued trade record must have a positive amount.");
            });
            l.transaction(tx -> {
                tx.command(Arrays.asList(TestUtils.ALICE.getPublicKey(), TestUtils.BOB.getPublicKey()),
                        new RecordedTradeContract.Commands.Issue());
